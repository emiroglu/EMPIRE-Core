...
CurveSurfaceCorotate3DMapper::CurveSurfaceCorotate3DMapper(int _curveNumNodes,
		int _curveNumElements, const double *_curveNodeCoors,
		const int *_curveNodeIDs, const int *_curveElems, int _surfaceNumNodes,
		const double *_surfaceNodeCoors, int _surfaceNumSections,
		...
		curveNumNodes(_curveNumNodes), curveNumElements(_curveNumElements), curveNodeCoors(
				_curveNodeCoors), curveElems(_curveElems), surfaceNumNodes(
				_surfaceNumNodes), surfaceNodeCoors(_surfaceNodeCoors), surfaceNumSections(
				_surfaceNumSections), surfaceNumRootSectionNodes(
	...
	assert(
			surfaceNumNodes
					== surfaceNumRootSectionNodes + surfaceNumTipSectionNodes
							+ (surfaceNumSections - 2)
									* surfaceNumNormalSectionNodes);
	...
	for (int i = 0; i < surfaceNumNodes; i++) {
		mapCoorX2Pos->insert(
				pair<double, int>(surfaceNodeCoorsInQ[i * 3 + 0], i));
	}

	int count = 0;
	for (multimap<double, int>::iterator it = mapCoorX2Pos->begin();
			it != mapCoorX2Pos->end(); it++) {
	...
	for (int i = 0; i < surfaceNumSections; i++) {
		...
		if (i == 0) { // root
			for (int j = 0; j < surfaceNumRootSectionNodes; j++) {
				sectionX += surfaceNodeCoorsInQ[sortedPosToUnsortedPos[j] * 3
						+ 0];
			}
		...
		double cubicShapeFuncDisp1 = 1.0 / 4.0 * (1.0 - xi) * (1.0 - xi)
				* (2.0 + xi);
		//double cubicShapeFuncRot1 = 1.0 / 8.0 * length * (1.0 - xi) * (1.0 - xi) * (1.0 + xi);
		double cubicShapeFuncRot1 = 1.0 / 8.0 * (1.0 - xi) * (1.0 - xi)
				* (1.0 + xi); // *=length
		double cubicShapeFuncDisp2 = 1.0 / 4.0 * (1.0 + xi) * (1.0 + xi)
				* (2.0 - xi);
		//double cubicShapeFuncRot2 = -1.0 / 8.0 * length * (1.0 + xi) * (1.0 + xi) * (1.0 - xi);
		double cubicShapeFuncRot2 = -1.0 / 8.0 * (1.0 + xi) * (1.0 + xi)
				* (1.0 - xi); // *=length
		//double Dxi_Dx = 2.0 / length;
		//double cubicShapeFuncDispDeriv1 = Dxi_Dx * (-3.0) / 4.0 * (1.0 - xi) * (1.0 + xi);
		double cubicShapeFuncDispDeriv1 = 2.0 * (-3.0) / 4.0 * (1.0 - xi)
				* (1.0 + xi); // /=length
		double cubicShapeFuncRotDeriv1 = 2.0 * (-1.0) / 8.0 * (1.0 - xi)
				* (1.0 + 3 * xi);
		//double cubicShapeFuncDispDeriv2 = Dxi_Dx * 3.0 / 4.0 * (1.0 - xi) * (1.0 + xi);
		double cubicShapeFuncDispDeriv2 = 2.0 * 3.0 / 4.0 * (1.0 - xi)
				* (1.0 + xi); // /=length
		double cubicShapeFuncRotDeriv2 = 2.0 * (-1.0) / 8.0 * (1.0 + xi)
				* (1.0 - 3 * xi);
			...
			sectionP[i * 3 + j] = shapeFuncOfSection[i * 10 + 0]
					* curveNodeCoors[curveNodeIDToPos->at(node1ID) * 3 + j]
					+ shapeFuncOfSection[i * 10 + 5 + 0]
							* curveNodeCoors[curveNodeIDToPos->at(node2ID) * 3
									+ j];
		}
	}
	...
	for (int i = 0; i < curveNumElements; i++) {
		...
		double length = sqrt(
				tmp[0] * tmp[0] + tmp[1] * tmp[1] + tmp[2] * tmp[2]); // do not use normalizeVector here
...
void CurveSurfaceCorotate3DMapper::consistentMapping(const double *curveDispRot,
	...
	for (int i = 0; i < curveNumElements; i++) {
		...
		int node2ID = curveElems[i * 2 + 1];
		for (int j = 0; j < 3; j++) {
			node1Disp[i * 3 + j] = curveDispRot[curveNodeIDToPos->at(node1ID)
					* 6 + 0 + j];
			node1Rot[i * 3 + j] = curveDispRot[curveNodeIDToPos->at(node1ID) * 6
					+ 3 + j];
			node2Disp[i * 3 + j] = curveDispRot[curveNodeIDToPos->at(node2ID)
					* 6 + 0 + j];
			node2Rot[i * 3 + j] = curveDispRot[curveNodeIDToPos->at(node2ID) * 6
					+ 3 + j];
		}
		...
		{ // compute ROT_elem_corotate, see Non-linear Modeling and Analysis of Solids and Structures (Krenk2009) P136
			const double *node1 = &curveNodeCoors[curveNodeIDToPos->at(node1ID)
					* 3];
			const double *node2 = &curveNodeCoors[curveNodeIDToPos->at(node2ID)
					* 3];
			...
			ROT_Eo_Er[i]->addRotation(corotateXAxis, corotateYAxis,
					corotateZAxis, true);
		}
		...
		{ // transform the DOFs to element local system
			...
			if (angleNode1 != 0.0) {
				...
				ROT_Oo_Ord__node1.addRotation(&node1Rot[i * 3], true,
						angleNode1);
				ROT_Eo_Erd__node1.addRotation(
						ROT_O_ELEM[i]->getRotationMatrix());
				ROT_Eo_Erd__node1.addRotation(
						ROT_Oo_Ord__node1.getRotationMatrix());
				ROT_Eo_Erd__node1.addRotation(ROT_ELEM_O->getRotationMatrix()); // O->E
				ROT_Eo_Ed__node1.addRotation(
						ROT_Eo_Erd__node1.getRotationMatrix());
			}
			...
			if (angleNode2 != 0.0) {
				...
				ROT_Oo_Ord__node2.addRotation(&node2Rot[i * 3], true,
						angleNode2);
				ROT_Eo_Erd__node2.addRotation(
						ROT_O_ELEM[i]->getRotationMatrix());
				ROT_Eo_Erd__node2.addRotation(
						ROT_Oo_Ord__node2.getRotationMatrix());
				ROT_Eo_Erd__node2.addRotation(ROT_ELEM_O->getRotationMatrix()); // O->E
				ROT_Eo_Ed__node2.addRotation(
						ROT_Eo_Erd__node2.getRotationMatrix());
			}
			...
			double torsionAngleAverage = (node1Rot[i * 3] + node2Rot[i * 3])
					/ 2.0;
	...
	for (int i = 0; i < surfaceNumSections; i++) {
		...
		sectionDisp[1] = cubicShapeFuncRot1 * node1Rot[elem * 3 + 2]
				+ cubicShapeFuncRot2 * node2Rot[elem * 3 + 2];
		//disp_z = NCr_1*(-rot_y1) + NCr_2*(-rot_y2), since disp_z' = -rot_y
		sectionDisp[2] = cubicShapeFuncRot1 * (-node1Rot[elem * 3 + 1])
		...
		sectionRot[1] = -(cubicShapeFuncRotDeriv1 * (-node1Rot[elem * 3 + 1])
				+ cubicShapeFuncRotDeriv2 * (-node2Rot[elem * 3 + 1]));
		//rot_z = disp_y' = NCr'_1*rot_z1 + NCr'_2*rot_z2
		sectionRot[2] = cubicShapeFuncRotDeriv1 * node1Rot[elem * 3 + 2]
		...
		for (int j = 0; j < 3; j++) {
			sectionDisp[j] = linearShapeFunc1 * node1Disp[elem * 3 + j]
		...
		KM_Eo_Erd.addTranslation(sectionDisp);
...
void CurveSurfaceCorotate3DMapper::conservativeMapping(
		const double *surfaceForce, double *curveForceMoment) {
	...
	for (int i = 0; i < surfaceNumSections; i++) {
		...
		for (int j = 0; j < numSectionNodes; j++) {
			...
			for (int k = 0; k < 3; k++) {
				curveForceMoment[node1Pos * 6 + 0 + k] += linearShapeFunc1
						* F[k];
				curveForceMoment[node1Pos * 6 + 3 + k] += linearShapeFunc1
						* M[k];
				curveForceMoment[node2Pos * 6 + 0 + k] += linearShapeFunc2
						* F[k];
				curveForceMoment[node2Pos * 6 + 3 + k] += linearShapeFunc2
						* M[k];
			}
		}
	}
...
double CurveSurfaceCorotate3DMapper::normalizeVector(double *vector) {
	double length = sqrt(
