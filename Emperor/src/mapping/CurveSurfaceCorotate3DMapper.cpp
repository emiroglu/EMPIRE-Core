...
CurveSurfaceCorotate3DMapper::CurveSurfaceCorotate3DMapper(int _curveNumNodes,
    ...
    /*
     * 2222222222222222DDDDDDDDDDDDDD
...
void CurveSurfaceCorotate3DMapper::consistentMapping(const double *curveDispRot,
    ...
    for (int i = 0; i < curveNumElements; i++) {
        ...
        { // transform the DOFs to element local system
            KinematicMotion *ROT_Erd_Ed = ROT_Eo_Er[i]->newInverse();
            ...
            ROT_Eo_Ed__node1.addRotation(ROT_Erd_Ed->getRotationMatrix()); // o_rd -> o_d
            ...
            ROT_Eo_Ed__node2.addRotation(ROT_Erd_Ed->getRotationMatrix()); // o_rd -> o_d

            // rotations related to deformation
    ...
    for (int i = 0; i < surfaceNumSections; i++) {
        ...
        // DOFs related to corotate
        // disp = NL_1*disp + NL_2*disp
        for (int j = 0; j < 3; j++) {
            sectionDisp[j] = linearShapeFunc1 * node1Disp[elem * 3 + j] + linearShapeFunc2 * node2Disp[elem * 3 + j];
        }
...
void CurveSurfaceCorotate3DMapper::conservativeMapping(const double *surfaceForce,
