...
const double IGAPatchSurface::EPS_CONVERGED_PROJECTION_ON_PATCH_BOUNDARY = 1e1;
...
void IGAPatchSurface::addTrimCurve(int direction, int _pDegree, int _uNoKnots, double* _uKnotVector,
    ...
    Trimming.addTrimCurve(direction, IDBasis, _pDegree, _uNoKnots, _uKnotVector, _uNoControlPoints,
            cpNet);
}
...
bool IGAPatchSurface::computePointProjectionOnPatchBoundaryOnGivenEdge(double& _t, double& _ratio,
    ...
    while (counter <= MAX_NUM_ITERATIONS) {
        ...
        if (df == 0)
            continue;
        if (fabs(f / df) < EPS_CONVERGED_PROJECTION_ON_PATCH_BOUNDARY)
            break;
...
bool IGAPatchSurface::computePointProjectionOnPatchBoundary(double& _u, double& _v, double& _ratio,
    ...
    for (int edge = 0; edge < 4; edge++) {
        ...
        for (int point = 0; point < 3; point++) {
            ...
            isConverged = computePointProjectionOnPatchBoundaryOnGivenEdge(t, div, distance, _P1,
                    _P2, edge);

            // Fix possible numerical error
            if (fabs(div - 1.0) < EPS_DISTANCE && div - 1.0 > 0)
                div = 1.0;
            if (fabs(div) < EPS_DISTANCE && div < 0)
                div = 0.0;
            ...
            if (isConverged || distance < _distance) {
                ...
                bool validPoint2 =
                        (point == 1) ?
                                (u[0] == u1 && v[0] == v1 && u[1] == u1 && v[1] == v1) : false; //Both are the same then true else false
                // If it is not a point to take into account, continue
                if (!(validPoint1 || validPoint2) && point < 2)
                    continue;
